...
class GameGrid:
    ...
    def neighbors(self, i, j):
        ...
        """
        neighbors=[(i-a, j-b) for a in range(-1,2) for b in range(-1,2)\
                   if i-a<self.nrows and j-b<self.ncols and i-a>=0 and j-b>=0]
    ...
    def flags_nearby(self, i, j):
        ...
        flags=[1 for a,b in self.neighbors(i, j) if self.flagged(a, b)]
        return len(flags)
    ...
    #For Exercise 4
    def cells(self):
        return [(i,j) for j in range(self.ncols) for i in range(self.nrows)]
    ...
    def random_game(self, n_mines):
        ...
        self.reset()
        for i,j in random.sample(self.cells(), n_mines):
            self.place_mine(i, j)
